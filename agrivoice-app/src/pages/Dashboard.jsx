...
export default function Dashboard() {
  ...
  const fetchAllData = async () => {
    try {
      setError(null)
      const [summaryData, diseasesData, impactData] = await Promise.all([
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-2xl font-bold text-gray-700">Loading Analytics...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-4xl font-bold text-green-700">📊 Impact Analytics Dashboard</h1>
          <p className="text-gray-600 mt-1">Real-time crop health monitoring for African farmers</p>
        </div>
        <div className="text-right">
          <p className="text-sm text-gray-500">Last updated:</p>
          <p className="font-semibold text-gray-700">{lastUpdated.toLocaleTimeString()}</p>
          <button
                ...
                {diseases.map((disease, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td className="text-center py-3 px-4">
                      <SeverityBadge breakdown={disease.severity_breakdown} />
                    </td>
                  </tr>
                ))}
              </tbody>
